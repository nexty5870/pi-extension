...
cmux placement follows explicit intent. The default is one right-side Team pane with implementer/reviewer terminal surfaces in the exact caller workspace and `--focus false`. `placement: new-window` creates a dedicated cmux window and Team workspace, stores its window/workspace/pane/surface references, and reattaches logs there even for a retained run. Single `cmux ...` CLI calls are topology operations—not project mutation—and remain available to the CTO agent; compound shell commands and direct code mutation stay blocked. State remains the control protocol; cmux displays/coalesces worker logs, status, progress, and flashes.
