...
A reportless `agent_settled` schedules one grace-period follow-up. If that follow-up also settles without `lead_worker_report`, the supervisor persists one ID-bearing runtime attention event; it never loops or infers completion. V2.1 keeps `reportBaselineAt` owned by agent start—`coordinator.report` never overwrites it—so a valid running or blocked report suppresses a reminder. Context warnings default to 80%; at 92% the worker receives one durable-handoff request without changing semantic status.
