...
`/workers` is the operator triage surface for inspect, inbox message/nudge, handoff request, explicit open/focus, graceful stop, exact-surface retire, session resume, and close-eligible actions. Focus only after the operator selects that action. Retirement never deletes session files or worktrees; blocked workers are not automatically retired. Capacity-full assignments remain durably queued and launch automatically when a slot opens.

`lead_delegate` may set an explicit `model` and `thinking`. Trusted private project policy can set project, role, and model-pattern defaults; precedence is explicit > model rule > role > project > inherited Lead. Treat the persisted resolved model/thinking as authoritative display evidence, while Pi performs capability clamping. Do not mutate global `models.json`.
